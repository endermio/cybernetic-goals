...
| Execution policy generation | `$superpowers:writing-plans` | Required for non-trivial execution policies | Cybernetic execution-policy skills provide semantic invariants, tactical freedom, dependency matrix, batch cadence, evidence check/evidence governance, and stale-evidence check policy as inputs. |
