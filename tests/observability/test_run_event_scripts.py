...
class RunEventScriptsTest(unittest.TestCase):
    ...
    def test_redactor_removes_nested_raw_input_and_completion_without_echoing_values(self):
        unsafe = json.loads(SAMPLE.read_text(encoding="utf-8"))
        unsafe["raw"] = {
            "input": "private input",
        ...
        redacted_event = payload["events"][0]
        self.assertEqual(redacted_event["raw"], {"message_count": 2})
        self.assertEqual(set(payload["redacted_fields"]), {"raw_completion", "raw_input"})
        output = result.stdout + result.stderr
    ...
    def test_sync_export_refuses_unredacted_event_with_nested_raw_input_and_completion(self):
        unsafe = json.loads(SAMPLE.read_text(encoding="utf-8"))
        unsafe["privacy_mode"] = "metadata_only"
        unsafe["raw"] = {"input": "private input", "completion": "private completion"}
