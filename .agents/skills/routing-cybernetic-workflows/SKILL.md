...
1. `Routing decision`
2. `Why`
3. `Rubric gate` when the task asks for evaluation, audit, readiness, closure, quality, completeness, correctness, usability, or status classification
4. `Recommended next step`
5. `Rejected workflow, if any`
...
If the task is evaluative and the rubric is missing or only partially defined, keep the execution-complexity level but add an `R` suffix:

- `Level 1R`: short rubric clarification before inline goal.
- `Level 2R`: rubric clarification before bounded file/audit goal.
- `Level 3`: full clarification must explicitly include rubric semantics.
...
If an audit/evaluation/status-classification task names labels such as `已闭环`, `部分闭环`, `未闭环`, or `不可判定` but does not define the evidence rubric for those labels, route to Level 2R before creating the bounded file goal.
...
### Level 2R: Bounded Goal With Rubric Clarification
...
Rubric clarity is orthogonal to the score. Missing evaluation rubric does not automatically mean Level 3; it means the selected level needs a rubric gate such as Level 1R or Level 2R unless the broader task already belongs in Level 3.
...
```markdown
Routing decision: Level 2R - Bounded Audit With Rubric Clarification
...
- [ ] For Level 1R/2R, the response recommends rubric clarification before execution.
