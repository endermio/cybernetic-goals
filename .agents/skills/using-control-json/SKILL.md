...
3. runtime writes only `progress.jsonl`, `runtime-status.json`, and `final-report.json`. Other generated evidence may exist only when the approved runtime JSON names it as non-control evidence.
