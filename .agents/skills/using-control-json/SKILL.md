...
5. `final-report.json` records the runtime completion claim and evidence; it does not grant verifier permission to itself. Before accepting or reporting `goal_achieved: true`, run the verifier as a structural final-claim gate. Verifier success is not a semantic quality gate; Counterexample Gate handles adversarial quality review.
