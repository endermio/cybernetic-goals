...
Approval requires a final observer pass after the last substantive mutation to the reviewed approved files.
...
Lint PASS is a structural evidence check, not meaning approval.

## Pre-goal Limit

Required Superpowers infrastructure must run before runtime `/goal` execution when it affects the approved work chain.

The runtime `/goal` must not create, approve, or rewrite its own approved work chain.
