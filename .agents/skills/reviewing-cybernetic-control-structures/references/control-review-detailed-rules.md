...
For `controlled_run` JSON pre-goal work, requirements analysis must contain `What the User Approved: Approved`.
...
Every approved review for JSON control runs must include a `counterexample-gate`
check. This is an independent reverse review: the reviewer attempts to disprove
...
- `reviewer.kind`: `subagent`, `human`, or `external`
- `reviewer.id`: the reviewer/session/approval identifier
- `reviewer.evidence_ref`: the transcript, review artifact, or approval
  reference
...
The reviewer asks whether any counterexample shows that:

- the target decomposition made the user's requested work easier, narrower, or
  merely classifiable;
- a required action became a support artifact such as a definition, plan,
  readiness result, or review report;
- blocked/stopped/no-more-work language lets unfinished required work count as
  complete;
- the runtime `/goal` can reach `goal_achieved: true` without producing the
  evidence the user asked for;
- an amendment patch changes strategy without fixing the observation that
  triggered it.

If a blocking counterexample exists, use `NeedsRevision` and route to the
earliest artifact that introduced the bad decomposition. If the counterexample
requires changing approved user meaning, use `Blocked` and report the smallest
human decision.
...
- `controlled_run` work assigns all target work to the main agent without a context-load justification;
