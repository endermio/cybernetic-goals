...
This skill is a thin orchestrator. It does not replace the other cybernetic skills. It coordinates them. Expanded chain orchestration is selected by `strategy_policy`, required design/review needs, or gate needs, not by legacy process-weight labels.
...
- `$routing-cybernetic-workflows` has recommended `controlled_run` JSON pre-goal orchestration with target model, strategy policy, and gate mode decisions;
...
- the task fits `ordinary_direct_work` or `bounded_runtime`;
...
After the routed revision, rerun independent review and Final Observer before runtime compilation. Until the review verdict is `Approved`, do not compile `runtime.control.json`.

Regression example: a required `/api/v2` implementation must not be accepted as legacy Drogon compatibility readiness. That is `NeedsRevision`, routed to the first artifact that converted implementation into compatibility readiness.
...
This is not Final Observer: it challenges the decomposition, work-package
assignment, runtime `/goal`, blocked claim, and completion claim. Blocking
counterexamples route to the earliest artifact that introduced the bad
decomposition.

An approved `counterexample-gate` must also include independent reviewer
provenance: `reviewer.kind` (`subagent`, `human`, or `external`), `reviewer.id`,
and `reviewer.evidence_ref`. A self-written evidence sentence does not prove the
gate ran.

## Finalize Approved Control Chain

Before `ReviewApproved -> RunRuntimeCompile`, all control JSON entering runtime
must be final. Generation-aware runs require `requirements.control.json`,
`run.control.json`, current `gen-N/runtime.control.json`, and named current
generation review to agree and pass validation. Expanded chains also require:
...
If review approves a candidate artifact, commit the approved chain before
runtime compile:
...
The second command must return `ok: true`. If either fails, stay in pre-goal
repair. Do not output the runtime `/goal`.
...
- any required `information_sufficiency_check` uses schema_version 1.2.0, is
  satisfied before entering design or execution policy, and passed independent
  counterexample review.

If requirements analysis is incomplete or What the User Approved is missing/not Approved, stop
before design.

If `information_sufficiency_check` exists in an older shape, lacks run-local
evidence, is not satisfied, or has no approved independent review, stop before
